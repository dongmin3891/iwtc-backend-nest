...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `10d0d7e`      |
...
프론트 디자인 작업은 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입과 `/manage` 생성 흐름의 기본정보·후보 추가·후보 카드·최종 저장 영역까지 완료했다. 다음 디자인 단위는 `/manage/[id]` 수정 화면의 전체 페이지 구조만 개선하는 것이다. 기존 기본정보 폼, 후보 편집기, 후보 카드와 저장 로직은 유지하면서 생성 화면과 같은 배경·헤더·단계 안내·반응형 2열 배치로 통일하고, 월드컵·후보 조회 중 로딩, 조회 실패·재시도와 빈 후보 상태를 명확히 한다. 후보별 기능이나 모바일 차단 정책 변경은 이후 단계로 분리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 관리 화면까지 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입과 `/manage` 생성 흐름의 기본정보·후보 추가·후보 카드·최종 저장 영역 디자인은 `10d0d7e`까지 완료했다. 다음 디자인 단위는 `/manage/[id]` 수정 화면의 전체 페이지 구조와 조회 로딩·실패 상태이며 기존 폼과 후보 저장 기능은 유지한다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
