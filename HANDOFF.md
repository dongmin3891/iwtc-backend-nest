...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `5de8846`      |
...
프론트 디자인 작업은 홈 화면과 `/play-game/[id]`의 게임 진입·라운드 선택 화면까지 완료했다. 다음 디자인 단위는 같은 경로에서 라운드 선택 후 표시되는 실제 1:1 대진 화면이다. 기존 후보 선택 애니메이션과 게임 진행 로직은 유지하면서 고정 `1600x800` 레이아웃을 반응형 구조로 바꾸고, 제목·현재 라운드·진행률·후보명·선택 가능 상태를 정리한다. `/play-clear/[...id]` 결과 화면은 그 다음 단계로 분리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 각 게임 화면의 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈과 게임 진입·라운드 선택 디자인은 `5de8846`까지 완료했으며 다음 디자인 단위는 `/play-game/[id]`의 실제 1:1 대진 화면이다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
