...
후보 일괄 생성 POST 경로의 실제 로컬 API와 PostgreSQL 검증까지 완료되었다. 프론트의 `createWorldCupContents` 호출 경로와 `{ data: [...] }` 요청 구조는 이미 신규 API와 일치한다. 다음 작업은 현재 백엔드가 유튜브 후보만 지원하는 동안 프론트 후보 입력 검증도 같은 범위로 제한하는 변경과 단위 테스트만 진행한다. 정적 이미지 후보가 서버로 전송되지 않도록 안내하고, HTTPS YouTube watch URL·5자리 시작 시간·3–5초 재생 시간 규칙을 프론트에서 확인한다. 이 단계에서는 UI 재설계, 브라우저 검증과 백엔드 수정은 진행하지 않는다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 신규 백엔드의 유튜브 후보 생성 규칙과 프론트 `src/domain/manage/content.ts`의 입력 검증·요청 매핑을 확인해줘. 기존 DB나 회원 데이터는 사용하지 않는다. 다음 단계에서는 백엔드를 건드리지 말고, 프론트에서 정적 이미지 후보 전송을 막고 HTTPS YouTube watch URL·5자리 시작 시간·3–5초 재생 시간을 검증하도록 도메인 검증 함수와 단위 테스트만 수정해줘. UI 재설계와 브라우저 검증은 다음 단계로 남겨줘.
