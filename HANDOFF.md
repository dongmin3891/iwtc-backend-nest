...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `33f2b5b`      |
...
프론트 디자인 작업은 홈 화면과 `/play-game/[id]`의 게임 진입·라운드 선택·실제 1:1 대진 화면까지 완료했다. 다음 디자인 단위는 `/play-clear/[...id]`의 우승 결과 화면이다. 결과 저장·랭킹·댓글 로직은 유지하면서 우승 후보 강조, 1~4위 결과, 다시 하기·홈 이동, 댓글 영역의 정보 구조와 반응형 배치를 정리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 결과·인증·관리 화면까지 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈과 게임 진입·라운드 선택·실제 1:1 대진 디자인은 `33f2b5b`까지 완료했으며 다음 디자인 단위는 `/play-clear/[...id]`의 우승 결과 화면이다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
