...
| ------------------ | ------------------------------------------------------ | --------------------------- | -------------- |
| 신규 백엔드        | `https://github.com/dongmin3891/iwtc-backend-nest.git` | `main`                      | `09e5d2b`      |
...
유튜브 후보 1개를 저장하는 내부 서비스도 구현되어 있다. 로그인 회원의 월드컵 소유권을 먼저 확인하고 `MediaFile`과 `Candidate`를 하나의 트랜잭션에서 생성한다. 후보 순서는 기존 마지막 `sortOrder` 다음 값이며, 후보가 없으면 0부터 시작한다. 아직 컨트롤러와 연결하지 않았으므로 외부에서 호출할 수 있는 후보 생성 API는 없다. 여러 후보의 일괄 저장과 동시 요청 충돌 처리는 아직 구현하지 않았다.
...
마지막 작업 기준으로 린트, 빌드, 단위 테스트 80개가 통과했다. API 통합 테스트는 직전 API 작업 기준 41개가 통과했으며, 이번 내부 서비스 작업에서는 다시 실행하지 않았다.
...
유튜브 후보 1개를 저장하는 내부 서비스까지 완료되었다. 다음 작업은 검증된 후보 배열을 하나의 트랜잭션에서 순서대로 저장하는 서비스만 구현한다. 기존 마지막 `sortOrder`를 한 번 조회하고 요청 배열 순서대로 연속된 값을 배정하며, 중간 후보 저장이 실패하면 해당 요청에서 만든 미디어와 후보가 모두 롤백되어야 한다. 이 단계에서도 컨트롤러, API 경로, 프론트 연결, 실제 PostgreSQL 검증, 동시 요청 충돌 처리는 진행하지 않는다.
