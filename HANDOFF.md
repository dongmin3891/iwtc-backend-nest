...
프론트 후보 생성 폼에서 정적 이미지 선택지를 숨기고 YouTube 전용 안내를 표시하는 작업까지 완료되었다. 다음 작업은 브라우저에서 후보 생성 폼의 표시 상태만 확인한다. 이미지 파일 버튼이 보이지 않고 안내 문구와 유튜브 영상 버튼이 표시되는지, 유튜브 영상 버튼을 누르면 주소·시작 시간·재생 시간 입력란이 기존처럼 열리는지만 검증한다. 이 단계에서는 실제 후보를 저장하거나 백엔드·프론트 코드를 수정하지 않는다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 프론트 후보 생성 폼의 YouTube 전용 UI 변경을 확인해줘. 기존 DB나 회원 데이터는 사용하지 않는다. 다음 단계에서는 코드를 수정하지 말고 브라우저에서 이미지 파일 버튼이 숨겨졌는지, YouTube 전용 안내와 유튜브 영상 버튼이 보이는지, 버튼을 누르면 주소·시작 시간·재생 시간 입력란이 열리는지만 확인해줘. 실제 후보 저장 흐름은 다음 단계로 남겨줘.
