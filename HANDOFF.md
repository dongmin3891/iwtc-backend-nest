...
`SelectVisibleType.tsx`의 라디오 입력 수정과 브라우저 재확인까지 완료되었다. 다음 작업은 실제 브라우저에서 빈 월드컵에 유효한 YouTube 후보 1개를 추가하고 `새로운 컨텐츠 생성 적용`으로 저장한 뒤, 관리 목록에 후보가 다시 표시되는지만 확인한다. 이 단계에서는 후보 생성 외의 백엔드·프론트 코드를 수정하지 않는다. 검증용 회원·세션·월드컵·후보·미디어는 확인 후 모두 삭제한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 DB나 회원 데이터는 사용하지 않는다. 다음 단계에서는 코드를 수정하지 말고 실제 브라우저에서 빈 월드컵에 유효한 YouTube 후보 1개를 추가한 다음 `새로운 컨텐츠 생성 적용`으로 저장하고, 관리 목록에 후보가 다시 표시되는지만 확인해줘. 검증용 회원·세션·월드컵·후보·미디어는 확인 후 모두 삭제해줘.
