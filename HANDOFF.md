...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `a73fe83`      |
...
프론트 디자인 작업은 홈 화면, `/play-game/[id]`의 게임 진입·라운드 선택·실제 1:1 대진, `/play-clear/[...id]`의 우승 결과·누적 랭킹·댓글, `/sign-in` 로그인, `/sign-up` 회원가입 화면까지 완료했다. 다음 디자인 단위는 `/manage` 관리 페이지의 전체 뼈대와 월드컵 기본정보 폼만 개선하는 것이다. 기존 생성 API와 후보 편집 상태 연결은 유지하면서 페이지 제목·작업 순서, 제목·설명·공개 설정, 생성 진행·성공·오류 상태와 반응형 배치를 정리한다. 후보 추가·수정 카드와 저장 영역은 이후 단계로 분리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 관리 화면까지 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈, 게임 진입·라운드 선택·실제 1:1 대진, 우승 결과·누적 랭킹·댓글, 로그인·회원가입 화면 디자인은 `a73fe83`까지 완료했다. 다음 디자인 단위는 `/manage` 관리 페이지의 전체 뼈대와 월드컵 기본정보 폼이며 후보 카드와 저장 영역은 건드리지 않는다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
