...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `94d96a3`      |
...
프론트 디자인 작업은 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입, `/manage`의 페이지 뼈대·기본정보 폼, 새 후보 추가 편집기와 추가된 후보 카드까지 완료했다. 다음 디자인 단위는 후보 목록의 최종 저장 영역만 개선하는 것이다. `WorldCupContentsManageListWrapper`의 제목·후보 개수·저장 버튼을 정리하고, 후보가 없거나 변경 사항이 없는 상태, 저장 중 중복 요청 방지, 성공·실패 피드백과 좁은 화면의 여백을 명확히 한다. `/manage/[id]` 수정 화면의 전체 페이지 구조는 그다음 단계로 분리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 관리 화면까지 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입과 `/manage`의 페이지 뼈대·기본정보 폼·새 후보 편집기·추가된 후보 카드 디자인은 `94d96a3`까지 완료했다. 다음 디자인 단위는 후보 목록의 최종 저장 영역이며 `/manage/[id]` 수정 화면 전체 구조는 건드리지 않는다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
