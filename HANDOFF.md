...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `38d245c`      |
...
프론트 디자인 작업은 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입, 월드컵 생성·수정과 내 월드컵 목록까지 완료했다. 다음 디자인 단위는 여러 화면에서 사용하는 공통 `AlertPopup`만 개선하는 것이다. 기존 `PopupContext` 호출 방식과 확인 동작은 유지하면서 배경 오버레이, 대화상자 구조, 메시지 가독성, 키보드 포커스와 390px 배치를 현재 다크 테마에 맞게 정리한다. 월드컵 삭제의 네이티브 확인창 통합, 모바일 차단 정책 변경과 다른 기능 수정은 이후 단계로 분리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 관리 화면까지 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입, 월드컵 생성·수정과 내 월드컵 목록 디자인은 `38d245c`까지 완료했다. 다음 디자인 단위는 공통 `AlertPopup`의 다크 테마·접근성·반응형 개선이며 기존 `PopupContext` 호출 방식과 확인 동작은 유지한다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
