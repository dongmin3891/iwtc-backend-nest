...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `e0c273e`      |
...
프론트 디자인 작업은 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입, 월드컵 생성·수정, 내 월드컵 목록과 공통 알림·확인 팝업까지 완료했고 모바일 전역 차단도 제거했다. 다음 디자인 단위는 후보 관리의 이미지 후보 카드와 YouTube 후보 카드에서 즉시 실행되는 삭제 동작을 공통 `ConfirmPopup`으로 통일하는 것이다. 새 후보와 저장된 후보의 기존 삭제 상태 추적, 최종 저장 방식과 API 계약은 유지하고, 두 카드의 삭제 문구·취소·확인 동작과 390px 배치만 정리한다. 다른 기능 수정은 이후 단계로 분리한다. 모바일 전체 페이지 회귀 검증은 별도 단계로 계속 진행한다.
