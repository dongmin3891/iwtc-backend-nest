...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `660317f`      |
...
프론트 디자인 작업은 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입과 `/manage`의 페이지 뼈대·월드컵 기본정보 폼까지 완료했다. 다음 디자인 단위는 새 후보 추가 편집기만 개선하는 것이다. `WorldCupContentsManageList`, `WorldCupContentCreateForm`, 미디어 유형 선택과 이미지·YouTube 입력 영역의 기존 검증·상태 연결을 유지하면서 후보 이름, 미디어 선택·미리보기, 공개 여부, 추가·취소 동작과 반응형 배치를 정리한다. 추가된 후보 카드, 후보 목록 저장 버튼과 `/manage/[id]` 수정 화면은 이후 단계로 각각 분리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 관리 화면까지 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입과 `/manage`의 페이지 뼈대·기본정보 폼 디자인은 `660317f`까지 완료했다. 다음 디자인 단위는 새 후보 추가 편집기이며 추가된 후보 카드, 목록 저장 버튼과 `/manage/[id]` 수정 화면은 건드리지 않는다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
