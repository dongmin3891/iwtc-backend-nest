...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `79379ca`      |
...
프론트 디자인 작업은 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입, `/manage`의 페이지 뼈대·기본정보 폼과 새 후보 추가 편집기까지 완료했다. 다음 디자인 단위는 편집기에 추가된 후보 카드만 개선하는 것이다. `ManageCardWrapper`, `StaticMediaFileTypeCard`, `InternetVideoUrlCard`의 기존 수정·삭제·상태 추적을 유지하면서 미디어 미리보기, 후보 이름·공개 상태, 편집·삭제 동작과 반응형 카드 배치를 정리한다. 후보 목록 최종 저장 버튼과 `/manage/[id]` 수정 화면의 페이지 구조는 이후 단계로 각각 분리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 관리 화면까지 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입과 `/manage`의 페이지 뼈대·기본정보 폼·새 후보 편집기 디자인은 `79379ca`까지 완료했다. 다음 디자인 단위는 추가된 후보 카드이며 후보 목록 최종 저장 버튼과 `/manage/[id]` 수정 화면 구조는 건드리지 않는다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
