...
| ------------------ | ------------------------------------------------------ | --------------------------- | -------------- |
| 신규 백엔드        | `https://github.com/dongmin3891/iwtc-backend-nest.git` | `main`                      | `077272c`      |
...
같은 월드컵의 후보 생성 요청을 PostgreSQL 트랜잭션 범위 advisory lock으로 직렬화하는 처리와 단위 테스트까지 완료되었다. 다음 작업은 실제 로컬 PostgreSQL에서 동일 월드컵에 두 개의 일괄 저장 요청을 동시에 실행해 충돌 없이 완료되는지 확인하는 검증만 진행한다. 두 요청의 후보가 중복 없는 연속 `sortOrder`로 저장되고 미디어 연결이 유지되는지 확인한 뒤, 만든 검증 데이터는 삭제한다. 이 단계에서도 컨트롤러, API 경로와 프론트 연결은 진행하지 않는다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 후보 일괄 저장 서비스의 advisory lock 처리와 테스트를 확인해줘. 기존 DB나 회원 데이터는 사용하지 않는다. 다음 단계에서는 API와 프론트를 건드리지 말고, 실제 로컬 PostgreSQL에 검증용 회원과 월드컵을 만든 다음 동일 월드컵에 두 일괄 저장 요청을 동시에 실행해 후보 순번과 미디어 연결을 검증해줘. 검증이 끝나면 만든 데이터를 모두 삭제하고 결과만 인수인계 문서에 기록해줘.
