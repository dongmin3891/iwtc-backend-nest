...
| ------------------ | ------------------------------------------------------ | --------------------------- | -------------- |
| 신규 백엔드        | `https://github.com/dongmin3891/iwtc-backend-nest.git` | `main`                      | `ad3b5e3`      |
...
유튜브 후보 배열을 하나의 트랜잭션으로 저장하는 내부 서비스까지 완료되었다. 다음 작업은 같은 월드컵에 후보 생성 요청이 동시에 들어왔을 때 `sortOrder` 고유 제약 충돌을 막는 처리와 단위 테스트만 추가한다. PostgreSQL 트랜잭션 안에서 월드컵별로 저장 순서를 직렬화한 뒤 마지막 `sortOrder`를 조회하도록 구현한다. 이 단계에서도 컨트롤러, API 경로, 프론트 연결과 실제 PostgreSQL 검증은 진행하지 않는다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 후보 일괄 저장 서비스와 테스트를 확인해줘. 기존 DB나 회원 데이터는 사용하지 않는다. 다음 단계에서는 API와 프론트를 건드리지 말고, 같은 월드컵에 후보 생성 요청이 동시에 들어와도 `sortOrder`가 충돌하지 않도록 PostgreSQL 트랜잭션 안에서 월드컵별 저장 순서를 직렬화하는 처리와 단위 테스트만 추가해줘. 실제 PostgreSQL 검증은 다음 단계로 남겨줘.
