...
게임 대진 조회에서는 요청 라운드와 비교하는 활성 공개 후보 수와 실제 무작위 대진에 사용하는 후보 목록에 모두 `deletedAt: null`을 적용했다. 다음 라운드의 탈락 후보 ID 제외 조건과 삭제 조건도 함께 전달되는 것을 단위 테스트로 확인했다. 전체 린트, 단위 테스트 113개, API 통합 테스트 50개, 빌드가 통과했다. 새 게임 결과 저장 후보 검증은 아직 변경하지 않았다.
...
Prisma 스키마와 공개 월드컵 미리보기·라운드 계산, 게임 대진 조회 단계는 완료되었다. 다음 한 단계에서는 `WorldCupsService.saveGameResult`의 새 게임 결과 후보 검증에만 `deletedAt: null` 조건을 추가하고 단위 테스트를 보강한다. 기존 `playId` 결과 재요청은 과거 결과 보존을 위해 현재 동작을 유지한다. 랭킹, 관리, 댓글, 삭제 API와 프론트엔드는 아직 수정하지 않는다.
...
> `iwtc-backend-nest/HANDOFF.md`와 `CANDIDATE_DELETION_POLICY.md`를 먼저 읽고 이어서 진행해줘. 기존 DB나 회원 데이터는 사용하지 않는다. 다음 단계에서는 `WorldCupsService.saveGameResult`의 새 게임 결과 후보 검증에만 `deletedAt: null` 조건을 추가하고 단위 테스트를 보강해줘. 기존 `playId` 결과 재요청은 현재 동작을 유지하고 랭킹, 관리, 댓글, 삭제 API와 프론트엔드는 아직 수정하지 마.
