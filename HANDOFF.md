...
게임 결과·랭킹·미디어·회원·비회원 댓글·회원 인증·회원 댓글 삭제·내 월드컵 목록·상세·생성·수정·영구 삭제·관리용 후보 목록·후보 생성·수정·삭제·정적 이미지 업로드와 교체 구현, 프론트 홈 화면 1차 디자인 개선은 각 원격 기준 브랜치에 push되어 있다. `iwtc.code-workspace`는 신규 백엔드 작업 트리에만 있는 로컬 편의 파일이며 커밋하지 않았다.
...
| DELETE | `/api/me/game-manage/world-cups/{worldCupId}`                 | 완료 |
...
마지막 작업 기준으로 프론트 타입 검사, 테스트 65개, 린트와 프로덕션 빌드가 통과했다. 저장소 이미지 표시용 `<img>`와 관련된 Next.js 린트 경고 2건이 관리 화면에 남아 있으나 실패는 아니다. 홈 화면 1차 디자인 개선에서는 공통 색상·배경, 반응형 헤더, 히어로, 검색·기간·정렬 필터와 월드컵 카드를 새 디자인으로 교체했고 좁은 화면 재배치도 브라우저에서 확인했다. 이어서 게임 진입·라운드 선택 화면을 전체 페이지 구조로 바꾸고 로딩, 조회 실패·재시도, 빈 라운드, 게임 시작 실패와 중복 선택 방지를 추가했다. 실제 1:1 대진 화면은 고정 `1600x800` 크기를 제거하고 반응형 후보 카드, 현재 라운드·남은 대결·진행률, 선택 잠금·다음 대결 로딩 상태를 갖춘 구조로 변경했다. 후보 영역은 키보드로도 선택 가능한 버튼이며 이미지·MP4·YouTube 미디어가 카드 크기를 채우도록 통일했다. 우승 결과 화면은 우승 후보 중심의 반응형 구성, 2~4위 결과, 다시 하기·홈 이동, 누적 인기 순위, 댓글 목록·입력과 각 로딩·실패 상태를 갖추도록 개편했다. 결과 화면의 일반 `<img>` 두 곳은 Next.js 이미지로 교체했다. 로컬 임시 API로 게임 결과·랭킹·댓글 데이터를 표시하고 댓글 입력 상태, 데스크톱과 좁은 화면 배치, 브라우저 오류·경고가 없음을 확인했다. 로그인 화면은 기존 인증·토큰·이동 로직을 유지하면서 반응형 2열 구성, 명시적 입력 라벨·검증 연결, 비밀번호 보기·숨기기, 로그인 중 중복 제출 방지, 회원가입·홈 이동을 갖추도록 개편했다. 회원가입 화면은 기존 API와 성공 후 이동 로직을 유지하면서 반응형 2열 안내·폼 구성, 명시적 라벨과 규칙 안내, 비밀번호·확인 값 보기 전환, 제출 중 중복 요청 방지, 로그인·홈 이동을 갖추도록 개편했다. `/manage` 생성 화면은 기본 정보와 후보 구성의 2단계 작업 구조, 반응형 2열 작업 공간, 제목·설명·공개 설정의 명시적 안내와 인라인 검증, 설명 글자 수, 생성 진행·완료·오류 상태를 갖추도록 개편했다. 새 후보 편집기는 기존 후보 데이터와 검증 연결을 유지하면서 이미지·YouTube 미디어 선택, 후보 이름, 반응형 이미지·영상 미리보기, 재생 구간, 공개 여부와 추가·취소 동작을 명시적인 입력 구조로 개편했다. 브라우저에서 생성 API만 모의해 실제 데이터를 만들지 않고 편집기를 열었으며 이미지 파일 미리보기, YouTube 입력과 재생 미리보기, 공개 설정, 취소 초기화와 데스크톱·390px 배치를 확인했다. 추가된 후보 카드는 기존 수정·삭제 상태 추적을 유지하면서 이미지·YouTube 미리보기, 후보 순서·이름·공개 상태, 재생 구간, 편집·취소·적용·삭제 동작을 반응형 카드로 개편했다. 모의 생성 응답과 클라이언트 상태로 이미지·YouTube 후보를 추가해 이미지 편집 취소, YouTube 이름·공개 상태 적용, 삭제 후 개수 갱신, 데스크톱·390px 배치를 확인했고 최종 저장 API는 호출하지 않았다. 이 변경은 프론트 `94d96a3`에 push되어 있다. 운영 배포 후 Next.js standalone 이미지에 Alpine용 `sharp` 네이티브 런타임이 누락된 문제를 발견했고, Docker runtime stage에 `sharp`, `@img`와 필요한 런타임 의존성을 명시적으로 복사하도록 수정했다. 수정 컨테이너에서 `/api/health`와 `/_next/image` 요청이 모두 HTTP 200인 것을 확인했고, 운영 주소에서도 이미지 최적화 요청을 네 번 확인해 모두 HTTP 200이었다. 해당 수정 커밋은 프론트 `16d6a1a`, 당시 자동 배포 커밋은 `43cc72a`다. `npm ci`에서 기존 의존성 취약점이 보고되었으며, 별도 검토 없이 강제 자동 수정하지 않는다.

후보 목록 최종 저장 영역은 후보 개수와 저장 전 변경 건수, 생성·수정별 안내, 빈 목록·변경 없음 비활성 상태, 저장 중 중복 요청 방지, 성공·실패 피드백을 하나의 반응형 패널로 정리했다. 기존 고정 높이 스크롤과 과도한 내부 여백을 제거했다. 생성 API와 후보 저장 API를 모의해 후보 0개, YouTube 후보 추가, 모의 실패 후 입력 유지·재시도, 모의 성공 후 홈 이동과 390px 한 열 배치를 확인했으며 실제 데이터는 생성하지 않았다. 이 변경은 프론트 `10d0d7e`에 push되어 있다.

`/manage/[id]` 수정 화면은 생성 화면과 같은 배경·헤더·단계 안내·반응형 2열 구조로 통일했다. 월드컵 상세, 후보 목록과 후보 미디어가 모두 준비된 뒤 편집기를 표시하며 초기 로딩 스켈레톤, 잘못된 ID·조회 실패·미디어 실패 안내와 다시 불러오기 동작을 추가했다. 모의 API로 지연 응답의 로딩 전환, 의도적 시간 초과 실패와 재시도 복구, 후보 0개 상태와 390px 한 열 배치를 확인했으며 실제 데이터는 사용하지 않았다. 이 변경은 프론트 `7b670c0`에 push되어 있다.

`/members/[id]/games` 내 월드컵 목록은 공통 배경·헤더, 공개 상태와 ID가 표시되는 반응형 카드 그리드, 플레이·후보 관리·삭제 동작으로 개편했다. 브라우저 마운트 이후 토큰을 읽어 서버·클라이언트 첫 화면의 hydration 불일치를 방지했고, 비로그인 안내, 로딩 스켈레톤, 조회 실패·재시도, 빈 목록과 삭제 실패 상태를 추가했다. 모의 API로 공개·비공개 카드 3개, 삭제 확인 후 3개에서 2개로 재조회, 빈 목록, 지연 로딩과 390px 한 열 배치를 확인했으며 실제 데이터는 삭제하지 않았다. 이 변경은 프론트 `38d245c`에 push되어 있다.
...
모바일 User-Agent를 감지해 `PC 버전으로 접속해주세요`만 표시하던 전역 `MobileView` 분기를 제거했다. 이제 모바일에서도 공통 Provider, 헤더와 실제 페이지가 렌더링되며, 더 이상 사용하지 않는 `react-device-detect` 의존성과 전용 컴포넌트도 삭제했다. iPhone 15 User-Agent와 393px 뷰포트에서 차단 문구 없이 반응형 홈 화면이 표시되는 것을 모의 API로 확인했다. 이 변경은 프론트 `e0c273e`에 push되어 있다.

이미지 후보 카드와 YouTube 후보 카드의 즉시 삭제 동작을 공통 `ConfirmPopup`으로 통일했다. 저장된 후보에는 아래 저장 버튼을 눌러야 실제 삭제가 반영된다고 안내하고, 아직 저장하지 않은 후보에는 목록에서 바로 제거된다고 구분해 표시한다. 기존 새 후보·저장 후보 삭제 상태 추적과 최종 저장 방식은 유지했다. 모의 데이터로 이미지 후보 취소, 저장된 YouTube 후보 삭제 대기 상태, 새 YouTube 후보 즉시 제거와 390px 배치를 확인했으며 실제 저장·삭제 API는 호출하지 않았다. 이 변경은 프론트 `727c682`에 push되어 있다.
...
- 프론트 `ajaxDelete`와 월드컵·후보·댓글 삭제 호출은 DELETE 요청 본문을 보내지 않는다. 이 수정은 프론트 `6abdd2e`까지 배포되었다.
- 백엔드 `05a6391`에 소유자 전용 `DELETE /api/me/game-manage/world-cups/{worldCupId}`를 구현했다. 성공 응답은 본문 없는 HTTP 204이며, 다른 회원 소유 또는 존재하지 않는 월드컵은 모두 HTTP 404다.
- 하나의 DB 트랜잭션에서 `GamePlacement` → `Comment` → `GamePlay` → `Candidate` → `WorldCup` 순서로 관계 데이터를 영구 삭제한다.
- 후보가 참조하던 미디어 중 삭제 후 다른 후보가 참조하지 않는 고아 `MediaFile`만 지우고 공유 미디어는 보존한다.
- MinIO의 `objectKey`와 `thumbnailObjectKey`는 DB 커밋 이후 중복을 제거해 삭제한다. 객체별 최대 3회 시도하며 최종 실패는 로그로 남기고 이미 완료된 DB 삭제 응답은 유지한다.
...
### 게임 선택 애니메이션 성능 개선 완료

- 프론트 기능 커밋은 `651696f`다.
- 프론트 게임 화면의 고정 1초 타이머와 최대 400px/2000px 카드 이동을 제거했다. 두 카드의 실제 애니메이션 완료를 기다린 뒤 다음 대결로 진행하며, 320ms duration과 32px/120px 이동·opacity·scale 조합을 사용한다.
- `prefers-reduced-motion`에서는 이동과 대기 시간을 제거한다. 좌·우 선택 방향, 입력 잠금, 승자·패자 계산과 다음 라운드 요청 순서는 유지했다.
- Playwright 모의 API로 실제 결과 데이터를 만들지 않고 390px 모바일과 1440px 데스크톱의 좌·우 선택을 확인했다. 실제 DOM 전환은 모바일 약 342ms, 데스크톱 약 336ms, reduced-motion 약 12ms였고 콘솔 오류는 없었다.
- 프론트 타입 검사, 린트, 테스트 69개와 프로덕션 빌드가 통과했다. 기능 커밋의 자동 배포가 끝난 뒤 실제 운영 모바일에서 한 번 더 체감 확인한다.
- `651696f` 운영 배포 후 탈락 카드의 `opacity: 0`과 transform이 다음 후보에도 남는 회귀 오류를 실제 WebKit 픽셀 캡처와 computed style로 확인했다. 접근성 후보명만 확인한 초기 검증으로는 발견되지 않았던 문제다.
- 프론트는 reset을 즉시 완료되는 spring update로 바꾸고 완료를 기다린 뒤 후보 목록을 교체한다. WebKit iPhone 15에서 다음 두 카드가 모두 `opacity: 1`, `transform: none`이고 실제 이미지가 표시되는 것을 확인했으며 타입 검사, 린트, 테스트 69개와 빌드가 통과했다. 수정은 `iwtc-frontend-new` `18bc2d6`로 커밋·push했다. 자동 배포가 끝난 뒤 운영 WebKit에서 실제 픽셀과 computed style을 다시 확인한다.
