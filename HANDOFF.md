...
| 프론트엔드         | `https://github.com/dongmin3891/iwtc-frontend-new.git` | `refactor/full-project`     | `7b670c0`      |
...
프론트 디자인 작업은 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입과 `/manage`, `/manage/[id]`의 생성·수정 흐름까지 완료했다. 다음 디자인 단위는 `/members/[id]/games` 내 월드컵 목록 화면만 개선하는 것이다. 기존 목록 조회·수정 이동·삭제와 재조회 로직은 유지하면서 페이지 헤더, 월드컵 카드, 공개 상태와 관리 동작, 반응형 카드 그리드를 정리하고 로딩·조회 실패·재시도·빈 목록 상태를 명확히 한다. 모바일 차단 정책 변경과 다른 화면 수정은 이후 단계로 분리한다. 전체 모바일 지원은 현재 `MobileView` 차단 정책을 제거하기 전에 관리 화면까지 반응형 동작을 별도로 검증해야 한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 운영 DB나 회원 데이터는 사용하지 않는다. 백엔드, 프론트엔드, PostgreSQL, MinIO의 홈서버 K3s 배포와 DNS, TLS, Argo CD 자동 동기화, 운영 사용자 흐름 검증까지 완료되었다. 프론트 홈, 게임, 결과·랭킹·댓글, 로그인·회원가입과 `/manage`, `/manage/[id]` 생성·수정 흐름 디자인은 `7b670c0`까지 완료했다. 다음 디자인 단위는 `/members/[id]/games` 내 월드컵 목록의 카드·반응형 배치와 로딩·실패·빈 상태이며 기존 조회·수정·삭제 기능은 유지한다. 인프라 다음 단계는 PostgreSQL `pg_dump`와 MinIO 객체를 Cloudflare R2로 보내는 외부 백업 구성, 보존 정책, 실제 복원 테스트다. 운영 Secret 값과 기존 `iwtc.code-workspace`는 커밋하지 마.
