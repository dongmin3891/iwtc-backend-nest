...
YouTube 후보 수정의 자동화 테스트와 실제 HTTP·PostgreSQL 검증까지 완료되었다. 다음 작업은 실제 브라우저에서 저장된 YouTube 후보 1개의 수정 화면을 열고 후보명, URL, 시작 시간, 반복 시간, 공개 여부를 변경한 뒤 `변경사항 적용`으로 저장하는 흐름만 확인한다. 관리 화면을 다시 열었을 때 모든 수정값이 유지되는지 확인하고, 이 단계에서는 백엔드·프론트 코드를 수정하지 않는다. 검증용 회원·세션·월드컵·후보·미디어는 확인 후 모두 삭제한다.
...
> `iwtc-backend-nest/HANDOFF.md`를 먼저 읽고 이어서 진행해줘. 기존 DB나 회원 데이터는 사용하지 않는다. 다음 단계에서는 코드를 수정하지 말고 실제 브라우저에서 저장된 YouTube 후보 1개의 후보명, URL, 시작 시간, 반복 시간, 공개 여부를 수정한 뒤 `변경사항 적용`으로 저장하고 관리 화면 재접속 후 모든 값이 유지되는지만 확인해줘. 검증용 회원·세션·월드컵·후보·미디어는 확인 후 모두 삭제해줘.
